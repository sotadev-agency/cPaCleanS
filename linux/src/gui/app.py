"""Interfaz gráfica de cPacleanS v3.0.2 — diseño profesional con tracker de fases (Linux)."""
import re
import subprocess
import threading
import traceback
...
class CpacleanSApp(ctk.CTk):
    ...
    def _open_pdf(self):
        if self._pdf_path and Path(self._pdf_path).exists():
            self._open_with_default_app(self._pdf_path)

    def _open_json(self):
        if self._json_path and Path(self._json_path).exists():
            self._open_with_default_app(self._json_path)

    @staticmethod
    def _open_with_default_app(path):
        """Abre un archivo con la app predeterminada del escritorio Linux
        (xdg-open, presente en GNOME/KDE/XFCE/etc.). Fallback a webbrowser."""
        try:
